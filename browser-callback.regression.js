...
(async () => {
  ...
  const relay = createCallbackServer({ context: { pages: () => [otherPage, page] }, port: 0 }); await relay.listen();
  try {
    let result = await request(relay, "POST", "/api/send"); assert.strictEqual(result.body.error, "send_guard_blocked");
    ...
    result = await request(relay, "POST", "/api/bind", { page_id: 2 }); assert.strictEqual(result.status, 200);
