...
if (require.main === module) { loadLocalEnvironment(); launchManualManagedBrowser().then((runtime) => { if (!runtime.disabled) { console.log(`[READY] visible managed Chromium is running; profile: ${runtime.profilePath}; CDP: http://127.0.0.1:${runtime.cdpPort}; start URL: ${runtime.startUrl}`); } }).catch((error) => { console.error(`[ERROR] ${error.message}`); process.exitCode = 1; }); }

module.exports = { launchManualManagedBrowser, loadLocalEnvironment, resolveManagedBrowserExecutable };
