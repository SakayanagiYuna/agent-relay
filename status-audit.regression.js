...
const done = buildStatusText({ status: "DONE", task, route, workerId: "worker-a", duration: "00m42s", gitAudit: audit, summary: "Implemented status audit.\nnpm test passed." });
for (const field of ["task_id: TASK-063", "status: DONE", "worker: worker-a", "workspace: baiyuan", "repo: agent-relay", "duration: 00m42s", "git_commit: abc1234", "changed_files: |\n  listener.js\n  status-audit.js", "git_diff_summary: 2 changed file(s), +12/-3", "test_result: passed"]) assert.ok(done.includes(field), `DONE must include ${field}`);
